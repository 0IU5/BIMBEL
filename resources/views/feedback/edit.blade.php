...
<div class="container flex justify-center items-center mx-auto min-h-screen">
    <div class="bg-white bg-opacity-60 backdrop-filter backdrop-blur-md shadow-xl rounded-lg p-8 sm:p-18 relative mx-auto w-full max-w-3xl" style="box-shadow: 0 0 20px 5px rgba(0, 255, 255, 0.5);">
        <h1 class="text-2xl font-bold mb-6 text-center text-gray-700">Edit Feedback</h1>
        <form action="{{ route('feedbacks.update', $feedback->id_feedback) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            <div>
                <label for="feedback" class="block text-sm font-medium text-gray-700">Feedback</label>
                <textarea name="feedback" id="feedback" rows="5" placeholder="Please Your Feedback..." class="input input-bordered input-info mt-1 block w-full h-40 p-4 @error('feedback') border-red-600 @enderror" required>{{ $feedback->feedback }}</textarea>
                @error('feedback')
